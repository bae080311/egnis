...
- 뽑아낸 실제 색상/폰트/spacing 값은 `.claude/design/figma-style-tokens.md`에 정리되어 있다. 시안의 "맛 선택" 서브패널처럼 우리 실제 옵션 데이터(개입수 suffix 구조)와 안 맞는 구조는 그대로 옮기지 않는다 — 그 파일의 "우리 구현에서 빼는 것" 항목 참고.
